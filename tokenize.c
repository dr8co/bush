...
void tokenize_redirect_input_output(char *cmdExec) {
    ...
    int m = 1;
    io_token[0] = strtok_skip(new_cmd_exec1, "<");
    free(new_cmd_exec1);
    while ((io_token[m] = strtok_skip(NULL, ">")) != NULL)
        m++;
...
void tokenize_redirect_input(char *cmdExec) {
    ...
    new_cmd_exec1 = _strdup(cmdExec);
    int m = 1;
    i_token[0] = strtok_skip(new_cmd_exec1, "<");
    while ((i_token[m] = strtok_skip(NULL, "<")) != NULL)
        m++;
...
void tokenize_redirect_output(char *cmdExec) {
    ...
    new_cmd_exec1 = _strdup(cmdExec);
    int m = 1;
    o_token[0] = strtok_skip(new_cmd_exec1, ">");
    free(new_cmd_exec1);
    while ((o_token[m] = strtok_skip(NULL, ">")) != NULL)
        m++;
